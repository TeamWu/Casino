...
def slots():
    ...
    def updateWinnings():
        ...
        totalStash = Label(root, text = "Cash =", width = 5, padx=5)
    ...
    def clickSpin(spin, sb, spinAgain, player, line1Var, line2Var, line3Var):
        #print("You have spun")
        spin.getRanNum()
        wheel1 = Label(root, text = spin.w1, width = 20).grid(row=1,column=2)
        wheel2 = Label(root, text = spin.w2, width = 20).grid(row=2,column=2)
        wheel3 = Label(root, text = spin.w3, width = 20).grid(row=3,column=2)
        #print("sb = ", sb)
    ...
    spinButton = Button(root, text= 'SPIN',fg = "white",command = clickSpin1,
                        background = "dark green",borderwidth=5, padx= 25).grid(
                            row=10,column=2)
    maxBet = Button(root, text= 'MAX BET',fg = "white", command = betMax,#allLines,
    ...
    quitButton = Button(root, text= 'quit',fg = "white", command= root.destroy,
                        background = "dark green",borderwidth=5, padx= 15).grid(
                            row=12,column=2)
    betLabel = Label(root, text = "BET:",width = 10).grid(row=15,column=2)
    
    betField = Spinbox(root, from_=1, to=player.getCash(),increment = 10,
                       validate="all",textvariable = selectBet).grid(row=16,column=2)

    line1 = Radiobutton(root, text='1', variable=line1Var, command = chooseLine1, value='1')
    line1.grid(row=1,column=4)

    line2 = Radiobutton(root, text='2', variable=line2Var, command = chooseLine2, value='2')
    line2.grid(row=2,column=4)

    line3 = Radiobutton(root, text='3', variable=line3Var, command = chooseLine3, value='3')
